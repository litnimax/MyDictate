        ...
        Form {
            ...
            Section("Частые слова и имена (подсказка распознаванию)") {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Через запятую или с новой строки. Передаются в AI-чистку (LM Studio) как подсказка — она исправляет похожие ошибки (напр. «Куля» → «Коля»). Требует включённой AI-обработки ниже.")
